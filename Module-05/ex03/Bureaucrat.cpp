...
/*   Updated: 2022/09/16 11:23:00 by klaarous         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Bureaucrat::signForm(Form &form)
{
	if (form.getIsSigned())
		...
		std::cout <<  "Bureaucrat " << _name << " could't sign  Form " << form.getName()  << " because the Form not signed"  << std::endl;
}

void	Bureaucrat::executeForm(Form const & form)
{
	if (form.isValidToExecute(*this))
	{
		form.execute(*this);
		std::cout << "bureaucrat " << this->_name  << " executed "  << form.getName() << std::endl;
	}
	else
		std::cout << "Failed to Execute this Form!!" << std::endl;
}
